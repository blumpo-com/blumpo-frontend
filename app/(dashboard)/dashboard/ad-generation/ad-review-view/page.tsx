...
function TinderPageContent() {
  ...
  // Mark ads as displayed when page loads (for quick ads only – when quick_ads=true in URL)
  useEffect(() => {
    // Create a unique key for this job+format combination
    const executionKey = `${jobId}-${formatParam}`;

    // Only run for quick-ads flow; don't run if we've already marked this job+format as displayed
    if (!isTest && isQuickAds && jobId && formatParam && hasMarkedAsDisplayedRef.current !== executionKey) {
  ...
  }, [jobId, formatParam, isTest, isQuickAds]);
