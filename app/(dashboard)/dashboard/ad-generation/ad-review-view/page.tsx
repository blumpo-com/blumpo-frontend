    ...
    if (!isTest && jobId && formatParam && hasMarkedAsDisplayedRef.current !== executionKey) {
      ...
      const markAdsAsDisplayed = async () => {
        try {
          ...
          // Only mark as displayed for quick ads jobs (charge on display: tokensCost 0).
          // Both auto-generated (paid) and manual (free) quick ads deduct tokens when ads are viewed.
          if (job.tokensCost > 0 && job.ledgerId) {
            await refreshUser();
            console.log('[AD-REVIEW] Job already charged (customized ads), skipping mark-displayed');
            return;
          }
