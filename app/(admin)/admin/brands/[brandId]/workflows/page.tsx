...
import {
  ...
  getWorkflowsNotUsedByBrand,
  getArchetypeActionCountsByBrand,
  getWorkflowActionCountsByBrand,
...
export default async function BrandWorkflowsPage({
  params,
  searchParams,
}: {
  ...
  const [brand, archetypeStats, workflowCounts, archetypes, workflowsNotUsed, archetypeActions, workflowActions] =
    await Promise.all([
      getBrandWithFullDetails(brandId),
      getArchetypeGenerationStatsByBrand(brandId),
      getWorkflowGenerationCountsByBrand(brandId),
      getAdArchetypes(),
      getWorkflowsNotUsedByBrand(brandId, archetypeFilter),
      getArchetypeActionCountsByBrand(brandId),
      getWorkflowActionCountsByBrand(brandId),
    ]);
  ...
  const titleLabel = brand.name;
  const reusedWorkflows = workflowCounts.filter((w) => Number(w.count) > 1);

  return (
    <div className="p-8">
      <div className="mb-6">
        <Link href={`/admin/brands/${brandId}`} className="text-blue-600 hover:text-blue-800">
          ← Back to Brand
        </Link>
      </div>
      ...
      <div className="space-y-6">
        ...
        <AdminCard title="Workflows not generated">
          <p className="text-sm text-gray-500 mb-4">
            Workflows in the catalog that this brand has never used. Filter by archetype below.
          </p>
          <form method="get" action={`/admin/brands/${brandId}/workflows`} className="mb-4 flex items-center gap-2">
          ...
          {workflowsNotUsed.length === 0 ? (
            <p className="text-sm text-gray-600">
              {archetypeFilter
                ...
                : 'Brand has generated with all workflow variants (or there are no workflows in the catalog).'}
            </p>
          ) : (
            <ul className="space-y-2">
