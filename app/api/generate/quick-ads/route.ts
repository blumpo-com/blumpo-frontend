...
// Token cost: same as customized-ads (single format 50, mixed 80)
function calculateQuickAdsTokenCost(formats: string[]): number {
  const hasSquare = formats.includes('1:1');
  const hasStory = formats.includes('9:16');
  if (hasSquare && hasStory) return 80;
  if (hasSquare || hasStory) return 50;
  return 50;
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Manual quick ads (autoGenerated false): deduct tokens immediately like customized-ads.
    // Auto-generated quick ads (autoGenerated true): no deduction here; tokens deducted when ads are viewed (mark-displayed).
    let tokensReserved = false;
    let tokensCost = 0;
    const isManualQuickAds = !job.autoGenerated;

    if (isManualQuickAds) {
      tokensCost = calculateQuickAdsTokenCost(job.formats || []);
      console.log('[GENERATE-QUICK-ADS] Manual quick ads – reserving tokens:', tokensCost, 'for formats:', job.formats);
      ...
      try {
        ...
        console.log('[GENERATE-QUICK-ADS] Tokens reserved for manual quick ads:', tokensCost);
      } catch (error) {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          return NextResponse.json({
            ...
            job_id: jobId,
            ...(tokensReserved ? { tokens_refunded: tokensCost } : {}),
          }, { status: res.status });
        }
      ...
      try {
        ...
        if (callbackResult.status === 'SUCCEEDED') {
          ...
          return NextResponse.json({
            ...
            ...(tokensReserved ? { tokens_used: tokensCost } : {}),
          });
        } else {
          // FAILED or CANCELED - refund if manual quick ads
          if (tokensReserved) {
            try {
              await refundTokens(user.id, tokensCost, jobId);
        ...
        return NextResponse.json({
          ...
          error_code: 'TIMEOUT',
          ...(tokensReserved && currentJob?.status !== 'SUCCEEDED' ? { tokens_refunded: tokensCost } : {}),
