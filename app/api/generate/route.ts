...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { url, brandId, autoGenerated } = body;
    console.log('[GENERATE] Received request:', { url, brandId, autoGenerated });
    ...
    // Validate input: either url (for free flow) or brandId (for quick ads)
    if (!url && !brandId) {
      console.error('[GENERATE] Missing both url and brandId');
      return NextResponse.json({ error: "Missing url or brandId" }, { status: 400 });
    }
    ...
    // Determine brand: use brandId if provided (quick ads), otherwise find by URL
    let existingBrand = null;
    if (brandId) {
      // For quick ads, use provided brandId
      const { getBrandById } = await import('@/lib/db/queries/brand');
      existingBrand = await getBrandById(brandId);
      if (!existingBrand || existingBrand.userId !== user.id) {
        return NextResponse.json({ error: "Brand not found or unauthorized" }, { status: 404 });
      }
      console.log('[GENERATE] Using provided brandId:', brandId);
    } else if (url) {
      // For free flow, find brand by website URL
      existingBrand = await getBrandByWebsiteUrl(user.id, url);
    ...
    try {
      ...
      // For quick ads: generate both formats (1:1 and 16:9) - 5 ads each = 10 total
      const formats = autoGenerated ? ['1:1', '16:9'] : [];
    ...
    try {
      ...
      const webhookPromise = fetch(webhookUrl, {
        ...
        body: JSON.stringify({
          job_id: jobId,
          website_url: url || null, // Pass website URL if available
          brand_id: brandId || null, // Pass brandId for quick ads
