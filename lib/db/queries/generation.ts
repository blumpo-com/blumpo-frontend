...
export async function createGenerationJob(
  ...
  return await db.transaction(async (tx) => {
    // Only deduct tokens at job creation when tokensCost > 0 (e.g. customized ads).
    // Quick ads (tokensCost 0) charge on display via mark-displayed; do not create a ledger entry here
    // or it would block the real -50 deduction (idempotency uses reason+referenceId).
    let ledgerEntry = null;
    if (!jobData.autoGenerated && jobData.tokensCost > 0) {
...
export async function getGenerationJobsForUser(
  userId: string, 
  limit = 20, 
  options?: {
    filterAutoGenerated?: boolean;
  }
) {
  let whereCondition;
  
  if (options?.filterAutoGenerated) {
    // For auto-generated quick ads, only include if they have at least one ready ad image and are succeeded
    whereCondition = and(
      eq(generationJob.userId, userId),
      or(
    ...
    whereCondition = eq(generationJob.userId, userId);
  }
