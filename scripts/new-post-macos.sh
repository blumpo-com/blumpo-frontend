...
REPO_ROOT=$(node "$SCRIPT_DIR/blog-utils.mjs" detect-root 2>/dev/null) || error "Not in a git repository"
...
PR_DATA=$(node "$SCRIPT_DIR/blog-utils.mjs" pr-summary "$MDX_PATH")
